...
public class Camera {
    ...
    public Camera(String cameraName, Transform3d robotToCameraTransform, Type type) {
        ...
        this.camera = new PhotonCamera(NetworkTableInstance.getDefault(), cameraName); // Initialize the PhotonCamera with the given NTI and name
        ...
        if (this.type == Type.APRIL_TAG) { // If the type is for AprilTags, initialize the pose estimator
            ...
            poseEstimator = new PhotonPoseEstimator( // Create a new PhotonPoseEstimator with the field layout and robot-to-camera transform
                aprilTagFieldLayout, 
                PoseStrategy.MULTI_TAG_PNP_ON_COPROCESSOR, // This is default, can change
                robotToCameraTransform
            );

            poseEstimator.setMultiTagFallbackStrategy(PoseStrategy.LOWEST_AMBIGUITY); // Set the fallback strategy for multi-tag estimation
        }
    ...
    private AprilTagFieldLayout loadField() {
        try { // Load the AprilTag field layout from the file
            String path = Paths.get(Filesystem.getDeployDirectory().getAbsolutePath(), "2025-reefscape-welded.json").toString();
            // String path = Paths.get(Filesystem.getDeployDirectory().getAbsolutePath(), Constants.FIELD_PATH).toString();
    ...
    public Pose3d getTagRelativeToBot(int tagID) {
        ...
        for (PhotonTrackedTarget target : latestResult.getTargets()) { // Iterate through each detected target
            if (target.getFiducialId() == tagID) { // If the target's ID matches the requested tagID
                ...
                System.out.println(cameraName + " " + cameraToTarget);
